...
class TaskSuite(Generic[Env]):
    ...
    def run_task_with_pipeline(
        ...
        if environment is None:
            environment = self.load_and_inject_default_environment(injections) # environment is injected with the injections
        ...
        model_output = None
        messages = []
        latest_extra_args: dict = {}
        for _ in range(3):
            # Run the user task
            try:
                _, _, task_environment, messages, latest_extra_args = agent_pipeline.query(prompt, runtime, task_environment)
            except AbortAgentError as e:
                # get last environment and messages
                task_environment = e.task_environment
                messages = e.messages
                if e.extra_args:
                    latest_extra_args = e.extra_args
        ...
        token_usage = latest_extra_args.get("token_usage") if isinstance(latest_extra_args, dict) else None
        if isinstance(token_usage, dict):
            logger = Logger.get()
            if hasattr(logger, "set_contextarg"):
                logger.set_contextarg("token_usage", copy.deepcopy(token_usage))
