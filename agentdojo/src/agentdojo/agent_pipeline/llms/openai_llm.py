...
def chat_completion_request(
    ...
    # create an extra api call for cot reasoning and print the response without interrupting the main api call
    # make the available tools to be text
    # add prompt to let the model generate thinking process for tool calling
    # then use the chat completion request to get the response without `tools` parameter
    # 把可用的 tool 转成文本，给思维链那次调用看
    if cot:
        if tools:
            tool_lines: list[str] = ["You have access to the following tools:"]
            for tool in tools:
                # ChatCompletionToolParam(type="function", function=FunctionDefinition(...))
                if tool.get("type", None) == "function" and tool.get("function", None) is not None:
                    name = tool.get("function", {}).get("name", "")
                    desc = tool.get("function", {}).get("description", "") or ""
                    tool_lines.append(f"- {name}: {desc}")
            tools_text = "\n".join(tool_lines)
        else:
            tools_text = "There are no tools available."

        cot_prompt = (
            "You are deciding which tool to call next given the user's goal and the conversation so far.\n"
            "Explain your WHOLE reasoning process for calling the tool in detail.\n"
            "Do not miss any steps in your reasoning process.\n\n"
            "Finally, clearly state the chosen tool name.\n"
            f"{tools_text}"
        )

        cot_messages: list[ChatCompletionMessageParam] = [
            ChatCompletionDeveloperMessageParam(
                role="developer",
                content=[ChatCompletionContentPartTextParam(type="text", text=cot_prompt)],
            ),
            *messages,
        ]

        # 这次调用不传 tools，只是让模型在看到工具列表文本的前提下输出思维链
    ...
    # print("[TOOL COT]", getattr(cot_completion.choices[0].message, "content", "No content"))
        return cot_completion
    else:
...
class OpenAILLM(BasePipelineElement):
    ...
    def query(
        ...
        # cot_completion = chat_completion_request(
        #     self.client, self.model, openai_messages, openai_tools, self.reasoning_effort, self.temperature, cot=True
        # )
        # cot_string = getattr(cot_completion.choices[0].message, "content", "")
        # output["cot"] = cot_string
