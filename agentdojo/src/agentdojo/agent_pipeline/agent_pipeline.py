...
class AgentPipeline(BasePipelineElement):
    ...
    def from_config(cls, config: PipelineConfig) -> Self:
        ...
        if config.defense == "spotlighting_with_delimiting":
            ...
            tool_output_formatter = lambda result: f"<<{base_formatter(result)}>>"
        ...
        if config.defense == "melon":
            ...
            pipeline.name = f"{llm_name}-{config.defense}"
            return pipeline
        if config.defense == "shadowmemory":
            from agentdojo.agent_pipeline.shadowmemory import ShadowMemory

            # agentdojo/ and memagent/ are siblings under the parent repo root
            # (the same layout shadowmemory.py relies on via sys.path injection).
            _default_mem_dir = Path(__file__).resolve().parents[3].parent / "memagent" / "mem_res" / "agentdojo_live"
            shadow = ShadowMemory(
                memory_dir=os.environ.get("SHADOWMEMORY_MEMORY_DIR", str(_default_mem_dir)),
                deny_mode="stop",
                base_url=os.environ.get("SHADOWMEMORY_BASE_URL", "http://localhost:8002/v1"),
            )
            tools_loop = ToolsExecutionLoop(
                [shadow, ToolsExecutor(tool_output_formatter), llm]
            )
            pipeline = cls([system_message_component, init_query_component, llm, tools_loop])
            pipeline.name = f"{llm_name}-{config.defense}"
            return pipeline
        if config.defense == "naivejudge":
            from agentdojo.agent_pipeline.naivejudge import NaiveJudge

            naive = NaiveJudge(
                deny_mode="stop",
                base_url=os.environ.get("NAIVEJUDGE_BASE_URL", "http://localhost:8001/v1"),
            )
            tools_loop = ToolsExecutionLoop(
                [naive, ToolsExecutor(tool_output_formatter), llm]
            )
            pipeline = cls([system_message_component, init_query_component, llm, tools_loop])
            pipeline.name = f"{llm_name}-{config.defense}"
            return pipeline
