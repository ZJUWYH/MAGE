...
def run_pipeline(args) -> list[PairResult]:
    ...
    if "vllm_parsed" in args.agent_model_name or args.agent_model_name == "local":
        resolved_model_name = _get_local_model_id(int(os.getenv("LOCAL_LLM_PORT", 8000))).replace("/", "_")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--defense", default=None, choices=[None, "tool_filter", "transformers_pi_detector", "spotlighting_with_delimiting", "repeat_user_prompt", "melon", "shadowmemory"], help="Defense method.")
