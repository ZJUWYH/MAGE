"""
Test JudgeAgentNaive with JudgeAgentNaiveWorkflow using AgentWorkflowEngine.

Uses the same dataset and output format as test.py, but excludes memory-related args.

Usage:
    python -m core.test_naive [--model MODEL] [--n_parallel N] [--indexes 4,5,6,7,8,9] [--split test|train] [--port PORT]
"""
...
from core.workflow import JudgeAgentNaiveWorkflow
...
async def run_test_naive(
    ...
    """
    Run evaluation on agentdojo dataset with JudgeAgentNaiveWorkflow.
    ...
    if dataset is None:
        print(f"[Test] ERROR: Dataset '{dataset_name}' split '{split}' not found. "
              f"Run 'python -m dataset.json_to_rllm_format --dataset_name {dataset_name}' first.")
        sys.exit(1)
    ...
    workflow_engine = AgentWorkflowEngine(
        workflow_cls=JudgeAgentNaiveWorkflow,
        workflow_args={"targeted_env": targeted_env},
    ...
    # ------------------------------------------------------------------
    # 4. Per-task output and aggregate metrics
    # ------------------------------------------------------------------
    all_metrics = []
...
def main():
    parser = argparse.ArgumentParser(description="Test JudgeAgentNaive with AgentWorkflowEngine")
    parser.add_argument("--model", type=str, default="gpt-5-mini",
                        help="OpenAI model name (default: gpt-5-mini)")
    parser.add_argument("--n_parallel", type=int, default=2,
                        help="Number of parallel workflow instances (default: 2)")
    parser.add_argument("--indexes", type=str, default=None,
                        help="Comma-separated sample indexes (e.g. 4,5,6,7,8,9)")
    ...
    parser.add_argument("--dataset_name", type=str, default="agentdojo",
