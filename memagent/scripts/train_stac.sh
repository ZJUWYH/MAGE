...
# screen -dmS train_stac bash -c "bash scripts/train_stac.sh > log/train_stac_${ts}.log 2>&1"
# in v2, we remove the memory reward and ablation reward, and increase the max response length to 8192
# in v3, we add the memory reward and ablation reward, add the tau length penalty
set -x
...
for gamma in "${GAMMAS[@]}"; do
    for tau in "${TAUS[@]}"; do
        CKPT_DIR="ckpt/Qwen3-4B-stacv2v3-gamma${gamma}-lambda${lambda}-tau${tau}-norm${normalize_reward}"
        MEMORY_DIR="mem_res/stacv2v3-train-gamma${gamma}-lambda${lambda}-tau${tau}-norm${normalize_reward}"
        EXP_NAME="Qwen3-4B-stacv2v3-gamma${gamma}-lambda${lambda}-tau${tau}-norm${normalize_reward}"
